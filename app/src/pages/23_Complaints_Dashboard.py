...
# Fetch complaints
try:
    res = requests.get("http://api:4000/ad/complaints")
...
if not complaints:
    ...
    for _, row in open_df.iterrows():
        with st.expander(f"#{row.TicketID} | {row.Category} | {row.Priority}"):
            ...
            try:
                email_resp = requests.get(f"http://api:4000/ad/customers/{row.CustomerID}/email")
            ...
            if st.button(f"Mark Ticket #{row.TicketID} as Resolved", key=row.TicketID):
                try:
                    resp = requests.put(f"http://api:4000/ad/complaints/{row.TicketID}/status")
